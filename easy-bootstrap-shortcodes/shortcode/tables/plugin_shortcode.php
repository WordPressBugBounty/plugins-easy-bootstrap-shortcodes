...
function osc_theme_os_table( $params, $content = null ) {
	...
	$width = Sanitizer::attr( $atts['width'] );
	$style = Sanitizer::custom_css( $atts['style'] );
	$class = Sanitizer::class_list( $atts['class'] );

	$out = '<table width="' . $width . '" class="table ' . $class . EBS_CONTAINER_CLASS . '" style="' . Sanitizer::attr( $style ) . '">' . do_shortcode( $content ) . '</table>';
	$out = 'true' === strtolower( $atts['responsive'] ) ? '<div class="table-responsive' . EBS_CONTAINER_CLASS . '">' . $out . '</div>' : $out;
